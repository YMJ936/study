...
public class MemberDAO { //비즈니스 로직빈
   ...
   //3.요구분석에 따른 웹상에서 호출할 메서드를 작성=>flow-chart
	...
	public MemberDTO getMember(String mem_id) {
		...
		try {
			...
			rs=pstmt.executeQuery();
			//id값에 해당되는 레코드를 찾아서 담기->if->한개이상 while(rs.next())
			if(rs.next()) {
				//찾은값 Setter의 매개변수로 저장->웹에 출력 Getter
				mem=new MemberDTO();
				mem.setMem_id(rs.getString("id"));//getMem_id()
			...
			System.out.println("getMember()호출에러=>"+e);
		}finally {
			pool.freeConnection(con, pstmt, rs);
		}
	...
	//6)찾은회원 수정=>회원가입해주는 메서드와 동일(sql구문만 다르다)
    ...
    boolean check=false;//회원수정 성공유무
		...
		try {
			...
			System.out.println("update(데이터수정유무)=>"+update);
			con.commit();
			if(update==1) {
				check=true;//데이터수정 성공확인
			}
		}catch(Exception e) {
	...
	public int memberDelete(String id,String passwd) {
		...
		try {
			...
			pstmt.setString(1, id); //index ~ 오류 뜨면 아 내가 물음표를 빼먹었구나
			rs=pstmt.executeQuery();
			//암호를 찾았다면
			if(rs.next()) {
				dbpasswd=rs.getString("passwd");
				System.out.println("dbpasswd=>"+dbpasswd);
				//dbpasswd(DB상 암호)==passwd(웹상에서 입력한 암호)
		...
		return x;//deletePro.jsp에서 memberDelete()에서 호출
	}
